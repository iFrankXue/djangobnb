...
const ReservationSidebar: React.FC<ReservationSidebarProps> = ({
    ...
    //             setFee(_fee);
    //             setTotalPrice((dayCount * property.price_per_night) + fee);
    //             setNights(dayCount);
    //         } else {
    ...
    useEffect(() => {
        ...
        if (dateRange.startDate && dateRange.endDate) {
            const dayCount = Math.max(differenceInDays(
                dateRange.endDate, 
                dateRange.startDate
            ), 0);
    
            if (property.price_per_night) {
                // Calculate fee directly in the scope of this effect
                const _fee = ((Math.max(dayCount, 1) * property.price_per_night) / 100) * 5;
    
                setFee(_fee);
                setTotalPrice((Math.max(dayCount, 1) * property.price_per_night) + _fee);
                setNights(Math.max(dayCount, 1));
            }
        }
    }, [dateRange, property.price_per_night]);

    const _setDateRange = (selection: { startDate: Date, endDate: Date }) => {
